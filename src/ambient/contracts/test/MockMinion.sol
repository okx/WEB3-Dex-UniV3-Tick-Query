...
contract MockMinion is ICrocMinion {
    ...
    function protocolCmd(uint16 proxyPath, bytes calldata cmd, bool sudo) public payable override {
    ...
    function userCmd(uint16 proxyPath, bytes calldata cmd) public payable returns (bytes memory) {
        paths_.push(proxyPath);
        userCmds_.push(cmd);
        callers_.push(tx.origin);
        return abi.encode();
...
contract MockMaster is ICrocMaster {
    ...
    function acceptsCrocAuthority() external pure override returns (bool) {
        return true;
    }
}
