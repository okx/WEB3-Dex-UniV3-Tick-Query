...
contract TestKnockoutLiq {
    ...
    function testEncodePivotKey(uint256 pool, bool isBid, int24 tick) public pure returns (bytes32) {
        return KnockoutLiq.encodePivotKey(bytes32(pool), isBid, tick);
    }

    function testEncodePosKey(uint256 pool, uint160 owner, bool isBid, int24 lower, int24 upper, uint32 pivotTime)
        public
        pure
        returns (bytes32)
    {
    ...
    function testCommit(uint96 lots, uint32 time, uint16 range, uint64 feeMileage) public {
    ...
    function testProof(uint160 root, uint256[] calldata proof) public view returns (uint32, uint64) {
        return merkle_.proveHistory(root, proof);
    }

    function testAssertValid(bool isBid, int24 lower, int24 upper, int24 priceTick, uint8 knockoutBits) public pure {
        KnockoutLiq.KnockoutPosLoc memory pos;
        pos.isBid_ = isBid;
        pos.lowerTick_ = lower;
        pos.upperTick_ = upper;
