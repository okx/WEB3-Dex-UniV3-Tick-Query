...
contract TestLiquidityCurve is LiquidityCurve {
    ...
    function liquidityReceivable(bytes32 poolIdx, uint128 liq, int24 lower, int24 upper)
        internal
        returns (uint128 base, uint128 flow)
    {
    ...
    function liquidityReceivable(bytes32 poolIdx, uint128 liq) internal returns (uint128 base, uint128 flow) {
        CurveMath.CurveState memory curve = snapCurve(poolIdx);
        (base, flow) = liquidityReceivable(curve, liq);
        commitCurve(poolIdx, curve);
    }

    function liquidityPayable(bytes32 poolIdx, uint128 liq, int24 lower, int24 upper)
        internal
        returns (uint128 base, uint128 flow)
    {
        CurveMath.CurveState memory curve = snapCurve(poolIdx);
    ...
    function liquidityPayable(bytes32 poolIdx, uint128 liq, uint64 rewards, int24 lower, int24 upper)
        internal
        returns (uint128 base, uint128 flow)
    {
        CurveMath.CurveState memory curve = snapCurve(poolIdx);
    ...
    function liquidityPayable(bytes32 poolIdx, uint128 liq) internal returns (uint128 base, uint128 flow) {
        CurveMath.CurveState memory curve = snapCurve(poolIdx);
        (base, flow) = liquidityPayable(curve, liq);
        commitCurve(poolIdx, curve);
    }

    function testLiqRecConc(uint256 poolIdx, uint128 liq, uint128 lower, uint128 upper) public {
        (baseFlow, quoteFlow) =
            liquidityReceivable(bytes32(poolIdx), liq, lower.getTickAtSqrtRatio(), upper.getTickAtSqrtRatio());
    }

    function testLiqRecTicks(uint256 poolIdx, uint128 liq, int24 lower, int24 upper) public {
    ...
    function testLiqPayConc(uint256 poolIdx, uint128 liq, uint128 lower, uint128 upper, uint64 rewards) public {
        (baseFlow, quoteFlow) =
            liquidityPayable(bytes32(poolIdx), liq, rewards, lower.getTickAtSqrtRatio(), upper.getTickAtSqrtRatio());
    }

    function testLiqPayTicks(uint256 poolIdx, uint128 liq, int24 lower, int24 upper) public {
    ...
    function testSwap(uint256 poolIdx, SwapAccum memory accum, uint128 bumpPrice, uint128 swapLimit) public {
        int24 bumpTick = TickMath.getTickAtSqrtRatio(bumpPrice);
        testSwapTick(poolIdx, accum, bumpTick, swapLimit);
    }

    function testSwapTick(uint256 poolIdx, SwapAccum memory accum, int24 bumpTick, uint128 swapLimit) public {
    ...
    function testSwapBumpInf(uint256 poolIdx, SwapAccum memory accum, uint128 swapLimit) public {
    ...
    function testSwapLimitInf(uint256 poolIdx, SwapAccum memory accum) public {
        int24 tick = accum.cntx_.isBuy_ ? TickMath.MAX_TICK : TickMath.MIN_TICK;
        uint128 limit = accum.cntx_.isBuy_ ? TickMath.MAX_SQRT_RATIO + 1 : TickMath.MIN_SQRT_RATIO - 1;
        testSwapTick(poolIdx, accum, tick, limit);
    }

    function fixCurve(uint256 poolIdx, uint128 price, uint128 ambientLiq, uint128 concLiq) public {
    ...
    function pullCurve(uint256 poolIdx) public view returns (CurveMath.CurveState memory) {
