...
contract TestAgentMaskRouter {
    ...
    function testRelayConds(address client, bool deadPast, bool aliveEarly, uint256 salt, uint32 nonce, address relayer)
        public
    {
        uint48 deadline = deadPast ? uint48(block.timestamp) - 1 : uint48(block.timestamp);
        uint48 alive = aliveEarly ? uint48(block.timestamp) + 1 : uint48(block.timestamp);
