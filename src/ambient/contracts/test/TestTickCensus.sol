...
contract TestTickCensus is TickCensus {
    ...
    function getBitmaps(int24 tick) public view returns (uint256 mezz, uint256 term) {
        mezz = mezzanineBitmap(poolIdx, tick);
        term = terminusBitmap(poolIdx, tick);
    }

    function testPinBuy(int24 tick) public view returns (int24, bool) {
        return pinBitmap(poolIdx, true, tick);
    }

    function testPinSell(int24 tick) public view returns (int24, bool) {
