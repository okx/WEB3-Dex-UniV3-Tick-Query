...
contract TestKnockoutCounter is KnockoutCounter {
    ...
    function testCross(uint256 pool, bool isBid, int24 tick, uint64 feeGlobal) public {
    ...
    function testMint(
        ...
        (pivotTime_, togglesPivot_) = addKnockoutLiq(bytes32(poolIdx), knockoutBits, tick, feeGlobal, loc, lots);

        callTime_ = uint32(block.timestamp);
    }

    // Mints a bid and an ask at the same time
    function testMintArch(
        uint256 poolIdx,
        uint8 knockoutBits,
        int24 tick,
        uint64 feeGlobal,
    ...
    function testBurn(uint256 poolIdx, int24 tick, uint64 feeGlobal, uint96 lots, bool isBid, int24 lower, int24 upper)
        public
    {
        KnockoutLiq.KnockoutPosLoc memory loc;
        loc.isBid_ = isBid;
        ...
        (togglesPivot_, pivotTime_, rewards_) = rmKnockoutLiq(bytes32(poolIdx), tick, feeGlobal, loc, lots);
    }
    ...
    function testClaim(
        ...
        (bookLots_, rewards_) = claimPostKnockout(bytes32(poolIdx), loc, merkleRoot, merkleProof);
    }

    function testRecover(uint256 poolIdx, bool isBid, int24 lower, int24 upper, uint32 pivotTime) public {
        KnockoutLiq.KnockoutPosLoc memory loc;
        loc.isBid_ = isBid;
        loc.lowerTick_ = lower;
        loc.upperTick_ = upper;
    ...
    function getPivot(uint256 poolIdx, bool isBid, int24 lower, int24 upper)
        public
        view
        returns (uint96 lots, uint32 pivotTime, uint16 range)
    {
        KnockoutLiq.KnockoutPosLoc memory loc;
        loc.isBid_ = isBid;
        loc.lowerTick_ = lower;
    ...
    function getMerkle(uint256 poolIdx, bool isBid, int24 lower, int24 upper)
        public
        view
        returns (uint160 root, uint32 pivotTime, uint64 feeMileage)
    {
        KnockoutLiq.KnockoutPosLoc memory loc;
        loc.isBid_ = isBid;
        loc.lowerTick_ = lower;
    ...
    function getPosition(uint256 poolIdx, bool isBid, int24 lower, int24 upper, uint32 pivotTime)
        public
        view
        returns (uint96 lots, uint64 feeMileage, uint32 timestamp)
    {
        KnockoutLiq.KnockoutPosLoc memory loc;
        loc.isBid_ = isBid;
        loc.lowerTick_ = lower;
