...
library Position {
    ...
    function _updateBoostedPosition(
        ...
        self.secondsDebtX96 =
            liquidityDelta > 0 ? self.secondsDebtX96 + secondsDebtDeltaX96 : self.secondsDebtX96 - secondsDebtDeltaX96; // can't overflow since each period is way less than uint31
    }
    ...
    function initializeSecondsStart(
        ...
        if (position.liquidity > 0) {
            (uint256 periodSecondsInsideX96, uint256 periodBoostedSecondsInsideX96) =
    ...
    /// @param params the position details and the change to the position's liquidity to effect
    function _updatePosition(UpdatePositionParams memory params)
        private
        returns (PositionInfo storage position, int128 boostedLiquidityDelta)
    {
...

        {
...

            {
                ...
                (boostedLiquidityCache.veRamRatio, boostedLiquidityCache.newBoostedLiquidity) = LiquidityMath
                    ...
                    newLiquidity, boostedPosition.veRamAmount, states.boostInfos[period].totalVeRamAmount
                );
                ...
                if (boostedLiquidityCache.newBoostedLiquidity > 0) {
                    ...
                    if (boostedLiquidityCache.veRamRatio < veRamUncapThreshold) {
                        ...
                        // check max balance allowed
                        {
                            uint256 maxBalance0 = LiquidityAmounts.getAmount0ForLiquidity(
                                midSqrtRatioX96, boostedLiquidityCache.upperSqrtRatioX96, type(uint128).max
                            );
                            uint256 maxBalance1 = LiquidityAmounts.getAmount1ForLiquidity(
                                boostedLiquidityCache.lowerSqrtRatioX96, midSqrtRatioX96, type(uint128).max
                            );
                        ...
                        uint256 boostedLiquidityCap =
                            FullMath.mulDiv(hypotheticalLiquidity, boostedLiquidityCache.veRamRatio, 1e18);
                    ...
                    {
                        // fetch existing data
                        veRamBoostCache.positionBoostUsedRatio = states.boostInfos[period].veRamInfos[params
                            .veRamTokenId].positionBoostUsedRatio[_positionHash];

                        veRamBoostCache.veRamBoostUsedRatio =
                            states.boostInfos[period].veRamInfos[params.veRamTokenId].veRamBoostUsedRatio;
                        ...
                        uint256 veRamBoostAvailableRatio =
                            1e18 > veRamBoostCache.veRamBoostUsedRatio ? 1e18 - veRamBoostCache.veRamBoostUsedRatio : 0;
                    ...
                    states.boostInfos[period].veRamInfos[params.veRamTokenId].veRamBoostUsedRatio =
                        uint128(veRamBoostCache.veRamBoostUsedRatio + veRamBoostCache.positionBoostUsedRatio);
                }
        ...
        {
            (uint160 secondsPerLiquidityPeriodX128, uint160 secondsPerBoostedLiquidityPeriodX128) =
                Oracle.periodCumulativesInside(uint32(period), params.tickLower, params.tickUpper);
    ...
    function _switchAttached(
        ...
        require(
            veRamTokenId == 0 || msg.sender == states.nfpManager
        ...
        if (veRamTokenId != oldAttached) {
            ...
            if (oldAttached != 0) {
                ...
                // update veRamBoostUsedRatio and positionBoostUsedRatio
                uint256 positionBoostUsedRatio =
                    states.boostInfos[period].veRamInfos[oldAttached].positionBoostUsedRatio[_positionHash];

                states.boostInfos[period].veRamInfos[oldAttached].veRamBoostUsedRatio -= uint128(positionBoostUsedRatio);
        ...
        returns (uint256 periodSecondsInsideX96, uint256 periodBoostedSecondsInsideX96)
    {
...

        {
            ...
            secondsPerLiquidityPeriodStartX128 =
                states.boostInfos[params.period].positions[_positionHash].secondsPerLiquidityPeriodStartX128;
            secondsPerBoostedLiquidityPeriodStartX128 =
                states.boostInfos[params.period].positions[_positionHash].secondsPerBoostedLiquidityPeriodStartX128;
        }

        (uint160 secondsPerLiquidityInsideX128, uint160 secondsPerBoostedLiquidityInsideX128) =
            Oracle.periodCumulativesInside(uint32(params.period), params.tickLower, params.tickUpper);
        ...
        secondsPerBoostedLiquidityInsideX128 =
            uint160(int160(secondsPerBoostedLiquidityInsideX128) - secondsPerBoostedLiquidityPeriodStartX128);
        ...
        if (boostedSecondsDebtX96 < 0 || periodBoostedSecondsInsideX96 > uint256(boostedSecondsDebtX96)) {
            periodBoostedSecondsInsideX96 =
                LiquidityMath.addDelta256(periodBoostedSecondsInsideX96, -boostedSecondsDebtX96);
        } else {
