...
library Oracle {
    ...
    function grow(Observation[65_535] storage self, uint16 current, uint16 next) external returns (uint16) {
        require(current > 0, "I");
        ...
        )
    {
        if (secondsAgo == 0) {
            ...
            return
                (last.tickCumulative, last.secondsPerLiquidityCumulativeX128, last.secondsPerBoostedLiquidityPeriodX128);
        }

        uint32 target = time - secondsAgo;

        (Observation memory beforeOrAt, Observation memory atOrAfter) =
            getSurroundingObservations(self, time, target, tick, index, liquidity, boostedLiquidity, cardinality);
        ...
        )
    {
        ...
        for (uint256 i = 0; i < secondsAgos.length; i++) {
            (tickCumulatives[i], secondsPerLiquidityCumulativeX128s[i], periodSecondsPerBoostedLiquidityX128s[i]) =
        ...
        )
    {
        ...
        secondsPerBoostedLiquidityCumulativeX128 = last.secondsPerBoostedLiquidityPeriodX128
        ...
        )
    {
        ...
        if (_slot0.tick < tickLower) {
            ...
            return (
                ...
                cache.secondsPerLiquidityCumulativeX128 - snapshot.secondsPerLiquidityOutsideLowerX128
                    - snapshot.secondsPerLiquidityOutsideUpperX128,
                cache.secondsPerBoostedLiquidityCumulativeX128 - snapshot.secondsPerBoostedLiquidityOutsideLowerX128
                    - snapshot.secondsPerBoostedLiquidityOutsideUpperX128,
    ...
    function periodCumulativesInside(
...

        {
            ...
            if (tickLower <= startTick && snapshot.secondsPerBoostedLiquidityOutsideLowerX128 == 0) {
                snapshot.secondsPerBoostedLiquidityOutsideLowerX128 =
                    states.periods[previousPeriod].endSecondsPerBoostedLiquidityPeriodX128;
            }
        ...
        if (lastTick < tickLower) {
            ...
            if (currentPeriod <= period) {
                ...
                (, cache.secondsPerLiquidityCumulativeX128, cache.secondsPerBoostedLiquidityCumulativeX128) =
                observeSingle(
                    states.observations,
                    cache.time,
                    0,
                ...
                cache.secondsPerBoostedLiquidityCumulativeX128 =
                    states.periods[period].endSecondsPerBoostedLiquidityPeriodX128;
            }
