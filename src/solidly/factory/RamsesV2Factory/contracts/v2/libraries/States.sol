...
library States {
    ...
    function balance0() internal view returns (uint256) {
        ...
        (bool success, bytes memory data) =
            states.token0.staticcall(abi.encodeWithSelector(IERC20Minimal.balanceOf.selector, address(this)));
    ...
    function balance1() internal view returns (uint256) {
        ...
        (bool success, bytes memory data) =
            states.token1.staticcall(abi.encodeWithSelector(IERC20Minimal.balanceOf.selector, address(this)));
        require(success && data.length >= 32);
        return abi.decode(data, (uint256));
    }
}
