...
contract RamsesV2Pool is IRamsesV2Pool, Initializable {
    ...
    function _advancePeriod() private {
        ...
        if ((States._blockTimestamp() / 1 weeks) != _lastPeriod) {
            ...
            ) = Oracle.newPeriod(states.observations, _slot0.observationIndex, period);
            ...
            states.periods[_lastPeriod].endSecondsPerLiquidityPeriodX128 = secondsPerLiquidityCumulativeX128;
            states.periods[_lastPeriod].endSecondsPerBoostedLiquidityPeriodX128 =
    ...
    function initialize(
        ...
        states.maxLiquidityPerTick = Tick.tickSpacingToMaxLiquidityPerTick(_tickSpacing);
    }
    ...
    function readStorage(bytes32[] calldata slots) external view override returns (bytes32[] memory returnData) {
    ...
    {
        PeriodBoostInfo storage periodBoostInfoData = States.getStorage().boostInfos[period];
        return (periodBoostInfoData.totalBoostAmount, periodBoostInfoData.totalVeRamAmount);
    }
    ...
    {
        BoostInfo memory boostInfo = States.getStorage().boostInfos[period].positions[key];
        return (boostInfo.boostAmount, boostInfo.veRamAmount, boostInfo.secondsDebtX96, boostInfo.boostedSecondsDebtX96);
    }
    ...
    function positionPeriodDebt(
        ...
        int24 tickLower,
        int24 tickUpper
    ) external view override returns (int256 secondsDebtX96, int256 boostedSecondsDebtX96) {
        States.PoolStates storage states = States.getStorage();
        BoostInfo storage position = Position.get(states.boostInfos[period], owner, index, tickLower, tickUpper);
    ...
    function positionPeriodSecondsInRange(
        ...
        int24 tickLower,
        int24 tickUpper
    ) external view override returns (uint256 periodSecondsInsideX96, uint256 periodBoostedSecondsInsideX96) {
        (periodSecondsInsideX96, periodBoostedSecondsInsideX96) = Position.positionPeriodSecondsInRange(
    ...
    {
        Observation memory observationData = States.getStorage().observations[index];
        return (
    ...
    function increaseObservationCardinalityNext(uint16 observationCardinalityNext) external override lock {
        ...
        uint16 observationCardinalityNextOld = states.slot0.observationCardinalityNext; // for the event
        uint16 observationCardinalityNextNew =
            Oracle.grow(states.observations, observationCardinalityNextOld, observationCardinalityNext);
    ...
    function initialize(uint160 sqrtPriceX96) external override {
        ...
        (uint16 cardinality, uint16 cardinalityNext) = Oracle.initialize(states.observations, 0);
    ...
    function mint(
        ...
        return mint(recipient, 0, tickLower, tickUpper, amount, type(uint256).max, data);
    }
    ...
    function mint(
        ...
        bytes calldata data
    ) public override lock advancePeriod returns (uint256 amount0, uint256 amount1) {
        require(amount > 0);
        ...
        IRamsesV2MintCallback(msg.sender).ramsesV2MintCallback(amount0, amount1, data);
        if (amount0 > 0) {
    ...
    function collect(
        ...
        return collect(recipient, 0, tickLower, tickUpper, amount0Requested, amount1Requested);
    }
    ...
    function collect(
        ...
        PositionInfo storage position = Position.get(states.positions, msg.sender, index, tickLower, tickUpper);
    ...
    function burn(
        ...
        (PositionInfo storage position, int256 amount0Int, int256 amount1Int) = Position._modifyPosition(
            Position.ModifyPositionParams({
                owner: msg.sender,
                index: index,
                tickLower: tickLower,
    ...
    function swap(
...

        {
            ...
            state = SwapState({
                ...
                feeGrowthGlobalX128: zeroForOne ? states.feeGrowthGlobal0X128 : states.feeGrowthGlobal1X128,
                protocolFee: 0,
                liquidity: cache.liquidityStart,
                boostedLiquidity: cache.boostedLiquidityStart,
                endSecondsPerLiquidityPeriodX128: states.periods[cache.previousPeriod].endSecondsPerLiquidityPeriodX128,
                endSecondsPerBoostedLiquidityPeriodX128: states.periods[cache.previousPeriod]
        ...
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            ...
            (step.tickNext, step.initialized) = TickBitmap.nextInitializedTickWithinOneWord(
                states.tickBitmap, state.tick, states.tickSpacing, zeroForOne
            );
            ...
            if (cache.feeProtocol > 0) {
                uint256 delta = (step.feeAmount * (cache.feeProtocol * 5 + 50)) / 100;
            ...
            if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
                ...
                if (step.initialized) {
                    ...
                    if (zeroForOne) {
                        ...
                        crossCache.feeGrowthGlobal0X128 = state.feeGrowthGlobalX128;
                        crossCache.feeGrowthGlobal1X128 = states.feeGrowthGlobal1X128;
                    } else {
                        crossCache.feeGrowthGlobal0X128 = states.feeGrowthGlobal0X128;
                        crossCache.feeGrowthGlobal1X128 = state.feeGrowthGlobalX128;
                    }
                    (crossCache.liquidityNet, crossCache.boostedLiquidityNet) = Tick.cross(
                        states._ticks,
                        Tick.CrossParams(
                            step.tickNext,
                    ...
                    if (zeroForOne) {
                        ...
                        crossCache.boostedLiquidityNet = -crossCache.boostedLiquidityNet;
                    }

                    state.liquidity = LiquidityMath.addDelta(state.liquidity, crossCache.liquidityNet);
                    state.boostedLiquidity =
                        LiquidityMath.addDelta(state.boostedLiquidity, crossCache.boostedLiquidityNet);
                }
        ...
        if (zeroForOne) {
            if (amount1 < 0) {
                TransferHelper.safeTransfer(states.token1, recipient, uint256(-amount1));
            }

            uint256 balance0Before = States.balance0();
            IRamsesV2SwapCallback(msg.sender).ramsesV2SwapCallback(amount0, amount1, data);
            require(balance0Before.add(uint256(amount0)) <= States.balance0(), "IIA");
        } else {
            if (amount0 < 0) {
                TransferHelper.safeTransfer(states.token0, recipient, uint256(-amount0));
            }

            uint256 balance1Before = States.balance1();
            IRamsesV2SwapCallback(msg.sender).ramsesV2SwapCallback(amount0, amount1, data);
            require(balance1Before.add(uint256(amount1)) <= States.balance1(), "IIA");
        }
    ...
    function flash(address recipient, uint256 amount0, uint256 amount1, bytes calldata data) external override lock {
        ...
        IRamsesV2FlashCallback(msg.sender).ramsesV2FlashCallback(fee0, fee1, data);
        ...
        if (paid.token0 > 0) {
            ...
            states.feeGrowthGlobal0X128 += FullMath.mulDiv(paid.token0 - fees0, FixedPoint128.Q128, _liquidity);
        }
        if (paid.token1 > 0) {
            ...
            states.feeGrowthGlobal1X128 += FullMath.mulDiv(paid.token1 - fees1, FixedPoint128.Q128, _liquidity);
        }

        emit Flash(msg.sender, recipient, amount0, amount1, paid.token0, paid.token1);
    }
    ...
    function collectProtocol(
        ...
        return ProtocolActions.collectProtocol(recipient, amount0Requested, amount1Requested);
    }
}
